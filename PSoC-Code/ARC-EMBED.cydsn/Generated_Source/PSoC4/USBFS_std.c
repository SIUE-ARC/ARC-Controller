...
uint8 USBFS_HandleStandardRqst(void) 
{
    ...
    uint32 bmRequestType = USBFS_bmRequestTypeReg;
...
void USBFS_ConfigReg(void) 
{
    ...
    USBFS_ARB_CFG_REG |= (uint32)  USBFS_ARB_CFG_CFG_CMP;
    ...
    USBFS_SIE_EP_INT_EN_REG = (uint32) USBFS_DEFAULT_SIE_EP_INT_EN;
}
    ...
    if (USBFS_configuration > 0u)
    {
        ...
        */
        ...
        for (i = 0u; i < ep; i++)
        {
            ...
            {
                curEp  = ((uint32) pEP->addr & USBFS_DIR_UNUSED);
                epType = ((uint32) pEP->attributes & USBFS_EP_TYPE_MASK);
            ...
            #if (USBFS_EP_MANAGEMENT_DMA_AUTO)
                /* Clear IN data ready. */
                USBFS_ARB_EP_BASE.arbEp[curEp].epCfg &= (uint32) ~USBFS_ARB_EPX_CFG_IN_DATA_RDY;
                ...
                if (0u != (pEP->addr & USBFS_DIR_IN))
                {
                    ...
                    USBFS_EP_TYPE_REG &= (uint32) ~(uint32)((uint32) 0x01u << (curEp - 1u));
                ...
                {
                    /* Set endpoint type: 0 - IN and 1- OUT. */
                    USBFS_EP_TYPE_REG |= (uint32) ((uint32) 0x01u << (curEp - 1u));
                ...
                */
                USBFS_DYN_RECONFIG_REG &= (uint32) ~USBFS_DYN_RECONFIG_ENABLE;
...
uint8 USBFS_ClearEndpointHalt(void) 
{
    ...
    if ((ep > USBFS_EP0) && (ep < USBFS_MAX_EP))
    {
        ...
        USBFS_SIE_EP_BASE.sieEp[ep].epCnt0 = (uint32) ~(uint32)USBFS_EPX_CNT_DATA_TOGGLE;
