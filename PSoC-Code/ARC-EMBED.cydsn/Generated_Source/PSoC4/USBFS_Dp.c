...
#include "USBFS_Dp.h"
    ...
    #define USBFS_Dp_SetP4PinDriveMode(shift, mode)  \
    do { \
        USBFS_Dp_PC =   (USBFS_Dp_PC & \
                                (uint32)(~(uint32)(USBFS_Dp_DRIVE_MODE_IND_MASK << \
                                (USBFS_Dp_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (USBFS_Dp_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define USBFS_Dp_SetP4PinDriveMode(shift, mode)  \
        do { \
            USBFS_Dp_USBIO_CTRL_REG = (USBFS_Dp_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(USBFS_Dp_DRIVE_MODE_IND_MASK << \
                                    (USBFS_Dp_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (USBFS_Dp_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		USBFS_Dp_SetP4PinDriveMode(USBFS_Dp__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(USBFS_Dp_DR & (uint8)(~USBFS_Dp_MASK));
    drVal = (drVal | ((uint8)(value << USBFS_Dp_SHIFT) & USBFS_Dp_MASK));
    USBFS_Dp_DR = (uint32)drVal;
}
...
{
    return (uint8)((USBFS_Dp_PS & USBFS_Dp_MASK) >> USBFS_Dp_SHIFT);
}
...
{
    return (uint8)((USBFS_Dp_DR & USBFS_Dp_MASK) >> USBFS_Dp_SHIFT);
}
...
void USBFS_Dp_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  USBFS_Dp_INTCFG & (uint32)(~(uint32)position);
    USBFS_Dp_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(USBFS_Dp_INTSTAT & USBFS_Dp_MASK);
	USBFS_Dp_INTSTAT = maskedStatus;
    return maskedStatus >> USBFS_Dp_SHIFT;
}
