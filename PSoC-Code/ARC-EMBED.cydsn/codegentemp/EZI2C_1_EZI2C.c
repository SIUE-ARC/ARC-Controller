...
***************************************/
    ...
    ****************************************************************************//**
    *
    *  Configures the EZI2C_1 for EZI2C operation.
    *
    *  This function is intended specifically to be used when the EZI2C_1 
    *  configuration is set to “Unconfigured EZI2C_1” in the customizer. 
    *  After initializing the EZI2C_1 in EZI2C mode using this function, 
    *  the component can be enabled using the EZI2C_1_Start() or 
    * EZI2C_1_Enable() function.
...
*  - EZI2C_1_EZI2C_STATUS_READ1 - Read transfer complete. The transfer 
...
*  - EZI2C_1_EZI2C_STATUS_WRITE1 - Write transfer complete. The buffer 
...
*  - EZI2C_1_EZI2C_STATUS_READ2 - Read transfer complete. The transfer
*    used the secondary slave address. The error condition status bit must be 
*    checked to ensure that read transfer was completed successfully.
*  - EZI2C_1_EZI2C_STATUS_WRITE2 - Write transfer complete. The buffer
*    content was modified. The transfer used the secondary slave address. 
*    The error condition status bit must be checked to ensure that write 
*    transfer was completed successfully.
*  - EZI2C_1_EZI2C_STATUS_BUSY - A transfer intended for the primary 
...
*  - EZI2C_1_EZI2C_STATUS_ERR - An error occurred during a transfer 
...
*    EZI2C_1_EZI2C_STATUS_ERR and EZI2C_1_EZI2C_STATUS_WRITE1/2 
*    is set. It is recommended to discard buffer content in this case.
*
* \globalvars
*  EZI2C_1_curStatus - used to store the current status of the EZI2C 
*  slave.
*
*******************************************************************************/
...
*
* \globalvars
*  EZI2C_1_addrBuf1 - used to store the primary 7-bit slave address 
*  value.
*
*******************************************************************************/
void EZI2C_1_EzI2CSetAddress1(uint32 address)
{
    #if(EZI2C_1_SECONDARY_ADDRESS_ENABLE)
    {
        ...
        EZI2C_1_RX_MATCH_REG = 
                        EZI2C_1_EzI2CUpdateRxMatchReg(address, (uint32) EZI2C_1_addrBuf2);
    }
    #else
    {
        uint32 matchReg;
        ...
        /* Set address. */
        matchReg &= (uint32) ~EZI2C_1_RX_MATCH_ADDR_MASK;
        matchReg |= (uint32)  EZI2C_1_GET_I2C_8BIT_ADDRESS(address);
...
*
* \globalvars
*  EZI2C_1_dataBuffer1 – the pointer to the buffer to be exposed to the
*  master on a primary address.
*  EZI2C_1_bufSizeBuf1 - the size of the buffer to be exposed to the 
*  master on a primary address.
*  EZI2C_1_protectBuf1 - the start index of the read-only region in the
...
void EZI2C_1_EzI2CSetReadBoundaryBuffer1(uint32 rwBoundary)
{
    ...
    *
    * \globalvars
    *  EZI2C_1_addrBuf2 - used to store the secondary 7-bit slave address 
    *  value.
    *
    *******************************************************************************/
    void EZI2C_1_EzI2CSetAddress2(uint32 address)
    {
        ...
        EZI2C_1_RX_MATCH_REG = 
                        EZI2C_1_EzI2CUpdateRxMatchReg((uint32) EZI2C_1_addrBuf1, address);
    }
    ...
    *
    * \globalvars
    *  EZI2C_1_dataBuffer2 – the pointer to the buffer to be exposed to the
    *  master on a secondary address.
    *  EZI2C_1_bufSizeBuf2 - the size of the buffer to be exposed to the 
    *  master on a secondary address.
    *  EZI2C_1_protectBuf2 - the start index of the read-only region in the
