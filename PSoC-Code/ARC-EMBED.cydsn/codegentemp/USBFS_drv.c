    ...
    if ((tempReg & USBFS_MODE_ACKD) != 0u)
    {
        ...
        if (modifyReg != 0u)
        {
            ...
            if ((tempReg & USBFS_MODE_SETUP_RCVD) == 0u)
            {
                ...
                tempReg = (uint32) USBFS_ep0Toggle | USBFS_ep0Count;
...
void USBFS_HandleSetup(void) 
{
    ...
    */
    ...
    USBFS_EP0_CR_REG = (uint32) requestHandled;
    requestHandled = (uint8) USBFS_EP0_CR_REG;
...
void USBFS_LoadEP0(void) 
{
    ...
    {
        USBFS_EP0_DR_BASE.epData[ep0Count] = (uint32) *USBFS_currentTD.pData;
...
void USBFS_NoDataControlStatusStage(void) 
{
    if (0u != USBFS_deviceAddress)
    {
        ...
        USBFS_CR0_REG = (uint32) USBFS_deviceAddress | USBFS_CR0_ENABLE;
