...
#include "USBFS_Dm.h"
    ...
    #define USBFS_Dm_SetP4PinDriveMode(shift, mode)  \
    do { \
        USBFS_Dm_PC =   (USBFS_Dm_PC & \
                                (uint32)(~(uint32)(USBFS_Dm_DRIVE_MODE_IND_MASK << \
                                (USBFS_Dm_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (USBFS_Dm_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define USBFS_Dm_SetP4PinDriveMode(shift, mode)  \
        do { \
            USBFS_Dm_USBIO_CTRL_REG = (USBFS_Dm_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(USBFS_Dm_DRIVE_MODE_IND_MASK << \
                                    (USBFS_Dm_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (USBFS_Dm_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		USBFS_Dm_SetP4PinDriveMode(USBFS_Dm__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(USBFS_Dm_DR & (uint8)(~USBFS_Dm_MASK));
    drVal = (drVal | ((uint8)(value << USBFS_Dm_SHIFT) & USBFS_Dm_MASK));
    USBFS_Dm_DR = (uint32)drVal;
}
...
{
    return (uint8)((USBFS_Dm_PS & USBFS_Dm_MASK) >> USBFS_Dm_SHIFT);
}
...
{
    return (uint8)((USBFS_Dm_DR & USBFS_Dm_MASK) >> USBFS_Dm_SHIFT);
}
...
void USBFS_Dm_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  USBFS_Dm_INTCFG & (uint32)(~(uint32)position);
    USBFS_Dm_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(USBFS_Dm_INTSTAT & USBFS_Dm_MASK);
	USBFS_Dm_INTSTAT = maskedStatus;
    return maskedStatus >> USBFS_Dm_SHIFT;
}
