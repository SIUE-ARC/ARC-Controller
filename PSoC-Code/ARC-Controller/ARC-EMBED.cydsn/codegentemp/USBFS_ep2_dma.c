...
uint8 USBFS_ep2_dma_initVar = 0u;


/*******************************************************************************
* Function Name: USBFS_ep2_dma_Start
********************************************************************************
*
* Summary:
...
{
    if(0u == USBFS_ep2_dma_initVar)
    {
        USBFS_ep2_dma_Init();
        USBFS_ep2_dma_initVar = 1u;
    }
    CyDmaEnable();
    USBFS_ep2_dma_SetSrcAddress(0, srcAddress);
    USBFS_ep2_dma_SetDstAddress(0, dstAddress);
    USBFS_ep2_dma_ValidateDescriptor(0);
    USBFS_ep2_dma_ChEnable();
}


/*******************************************************************************
* Function Name: USBFS_ep2_dma_Init
********************************************************************************
*
* Summary:
*  Initializes the DMA channel based on the parameters set in the component
*  customizer. It is not necessary to call USBFS_ep2_dma_Init() because the
*  USBFS_ep2_dma_Start() API calls this function, which is the preferred method
...
void USBFS_ep2_dma_Init(void)
{
    ...
    CYDMA_DESCR_BASE.descriptor[USBFS_ep2_dma_CHANNEL][0].ctl = USBFS_ep2_dma_DESCR0_CFG;
    CYDMA_DESCR_BASE.descriptor[USBFS_ep2_dma_CHANNEL][1].ctl = USBFS_ep2_dma_DESCR1_CFG;

    /* If the Generate IRQ option is selected for descriptor 0 or 1, set the
    * channel bit field in the DMA interrupt mask register.
    */
    #if (0u != ((USBFS_ep2_dma_DESCR0_CFG | USBFS_ep2_dma_DESCR1_CFG) & CYDMA_GENERATE_IRQ))
        CYDMA_INTR_MASK_REG |= USBFS_ep2_dma_CHANNEL_MASK;
    #endif /* (0u != ((USBFS_ep2_dma_DESCR0_CFG | USBFS_ep2_dma_DESCR1_CFG) & CYDMA_GENERATE_IRQ)) */
}
