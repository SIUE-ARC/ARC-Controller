...
void USBFS_Stop(void) 
{
    ...
    USBFS_CR0_REG &= (uint32) ~USBFS_CR0_ENABLE;

    /* Disable D+ pull-up. */
    USBFS_USBIO_CR1_REG &= (uint32) ~ USBFS_USBIO_CR1_USBPUEN;
...
{
    uint32 cr1Reg = USBFS_CR1_REG;

    /* Clear bus activity. */
    USBFS_CR1_REG = (cr1Reg & (uint32) ~USBFS_CR1_BUS_ACTIVITY);
